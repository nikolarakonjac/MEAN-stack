...
export class DodajDekorateraComponent {
  constructor(private korisniciServis: KorisniciService, private router: Router){}


  ngOnInit(): void{
    this.dekorater.tipKorisnika = "dekorater"
  }
    ...
    if (file) {
      ...
      img.onload = () => {
        ...
        if (width >= 100 && width <= 300 && height >= 100 && height <= 300) {
          this.dekorater.profilnaSlika = file;
  ...
  onSubmit(): void {
    if(!this.validatePassword(this.dekorater.lozinka)){
    ...
    if (!this.validateCreditCard(this.dekorater.kreditnaKartica)) {
    ...
    if (!this.validatePhoneNumber(this.dekorater.telefon)) {
    ...
    if (!this.validateEmail(this.dekorater.mejl)) {
      this.errorMessageMejl = 'Uneta mejl adresa nije validna.';
      return;
    }
    this.errorMessageMejl = ""

    this.korisniciServis.dodajNovogDekoratera(this.dekorater).subscribe(
      data=>{
        if(data.poruka === "ok"){
